...
| enterprise.privateNetworks.externalEndpoints.enabled | bool | `false` |  |
...
| enterprise.waf.crs | object | `{"image":{"digest":"","override":null,"pullPolicy":"IfNotPresent","repository":"","tag":"","useDigest":false},"sourcePath":"/etc/coraza/crs"}` | Global CRS override for WAF. When image.repository or image.override is set, cilium-envoy mounts CRS from this image instead of the bundled CRS. |
| enterprise.waf.crs.sourcePath | string | `"/etc/coraza/crs"` | Path in the CRS image that contains the CRS root directory. The directory must contain rules/ and either crs-setup.conf or crs-setup.conf.example. |
...
| envoy.affinity | object | `{"nodeAffinity":{"requiredDuringSchedulingIgnoredDuringExecution":{"nodeSelectorTerms":[{"matchExpressions":[{"key":"cilium.io/no-schedule","operator":"NotIn","values":["true"]}]}]}},"podAffinity":{"requiredDuringSchedulingIgnoredDuringExecution":[{"labelSelector":{"matchLabels":{"k8s-app":"cilium"}},"topologyKey":"kubernetes.io/hostname"}]},"podAntiAffinity":{"preferredDuringSchedulingIgnoredDuringExecution":[{"podAffinityTerm":{"labelSelector":{"matchLabels":{"k8s-app":"cilium-envoy"}},"topologyKey":"kubernetes.io/hostname"},"weight":100}],"requiredDuringSchedulingIgnoredDuringExecution":[]}}` | Affinity for cilium-envoy. |
...
| envoy.gracefulRestart | object | `{"drainTimeSeconds":60,"enabled":false,"parentShutdownSeconds":80}` | Configure Cilium Envoy graceful shutdown. |
| envoy.gracefulRestart.drainTimeSeconds | int | `60` | The time in seconds that Envoy will drain connections during a graceful restart. Should generally be less than `parentShutdownSeconds` and `envoy.terminationGracePeriodSeconds` |
...
| envoy.gracefulRestart.parentShutdownSeconds | int | `80` | The time in seconds that Envoy will wait before shutting down the parent process during a graceful restart. Should generally be less than `envoy.terminationGracePeriodSeconds` |
...
| hubble.export.timescape.batchFlushInterval | string | `250ms` | The maximum time to wait before flushing a partial flow batch. @schema type: [null, string] @schema |
| hubble.export.timescape.batchSize | string | `256` | The maximum number of flows to send in a single batch. @schema type: [null, integer] @schema |
...
| hubble.export.timescape.ingestMode | string | `auto` | Timescape ingest RPC mode to use. Supported values are `auto`, `batch`, and `single`. @schema type: [null, string] @schema |
...
| hubble.export.timescape.target | Deprecated | `hubble-timescape-export.hubble-timescape.svc.cluster.local:4261` | The target server to connect to for exporting flows. This field is deprecated in favor of `targets` (plural) which supports multiple Timescape endpoints. If set, this value is merged into `targets` (appended and deduplicated). @schema type: [null, string] @schema |
| hubble.export.timescape.targets | string | `["hubble-timescape-export.hubble-timescape.svc.cluster.local:4261"]` | The target servers to connect to for exporting flows. Supports sending Hubble flow data to multiple Timescape endpoints. The deprecated singular `target` field, if set, is merged into this list (appended and deduplicated). Example: `["timescape1.example.com:4261", "timescape2.example.com:4261"]` @schema type: [null, array] @schema |
...
| hubble.rbac | object | `{"auth":{"oidc":{"ca":{"configMap":{"key":"ca.crt","name":null}},"clientID":null,"issuerUrl":null}},"enabled":false,"extraArgs":{},"extraEnv":[],"healthCheckPort":8079,"image":{"digest":"sha256:743a789d406b5e30be0573511d3f42570dffcbcc948e3ddf069fdb6816e8b6ca","override":null,"pullPolicy":"Always","repository":"quay.io/isovalent/hubble-rbac","tag":"v1.10.11","useDigest":true},"listenPort":4245,"loggingLevel":"info","policy":{"configMap":{"bindings":[],"create":true,"key":"hubble-rbac-policy.yaml","name":"hubble-rbac-policy","roles":[]},"logRoles":false},"resources":{},"securityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"privileged":false,"runAsGroup":65532,"runAsNonRoot":true,"runAsUser":65532},"useKubernetesGrpcProbes":true}` | Enables experimental support for using Hubble RBAC with Hubble Relay. This is incompatible with Hubble RBAC installed via the Hubble Enterprise chart. |
...
| hubble.timescape.annotations | object | `{}` | Annotations to add to all Hubble Timescape resources. |
| hubble.timescape.clickhouse.configMap.annotations | object | `{}` | Annotations to add to the Hubble Timescape ClickHouse ConfigMap. |
| hubble.timescape.clickhouse.configMap.labels | object | `{}` | Labels to add to the Hubble Timescape ClickHouse ConfigMap. |
| hubble.timescape.clickhouse.extraVolumeMounts | list | `[]` | Extra volume mounts for the Hubble Timescape Clickhouse container |
...
| hubble.timescape.clustermesh | object | `{"primary":{"annotations":{},"createNamespace":false,"id":0,"labels":{},"namespace":""}}` | Experimental support for using Hubble Timescape with clustermesh. If enabled, all flow log will be sent to the `primary` Timescape instance. |
| hubble.timescape.clustermesh.primary.annotations | object | `{}` | Annotations to add to the Hubble Timescape primary namespace. |
| hubble.timescape.clustermesh.primary.labels | object | `{}` | Labels to add to the Hubble Timescape primary namespace. |
| hubble.timescape.configMap.annotations | object | `{}` | Annotations to add to the Hubble Timescape ConfigMap. |
| hubble.timescape.configMap.labels | object | `{}` | Labels to add to the Hubble Timescape ConfigMap. |
| hubble.timescape.extraEnv | list | `[]` | Extra environment variables for hubble-timescape. |
| hubble.timescape.extraVolumeMounts | list | `[]` | Extra volume mounts for the Hubble Timescape container |
| hubble.timescape.extraVolumes | list | `[]` | Extra volumes for the Hubble Timescape pod |
| hubble.timescape.grafana | object | `{"dashboards":{"annotations":{},"enabled":false,"labels":{"grafana_dashboard":"1"},"namespace":null}}` | Optional ConfigMap with Grafana dashboards for Hubble Timescape. Grafana can import dashboards based on the labels. ref: https://github.com/grafana/helm-charts/tree/main/charts/grafana#sidecar-for-dashboards |
| hubble.timescape.grafana.dashboards.annotations | object | `{}` | Annotations to add to the ConfigMap. |
| hubble.timescape.grafana.dashboards.labels | object | `{"grafana_dashboard":"1"}` | Labels to add to the ConfigMap. If using the dashboards sidecar, they must include the label used to discover dashboards. |
| hubble.timescape.grafana.dashboards.namespace | string | `nil` | Namespace to create the ConfigMap in. Defaults to namespace of the helm release. @schema type: [null, string] @schema |
| hubble.timescape.headlessService.annotations | object | `{}` | Annotations to add to the Hubble Timescape headless service. |
| hubble.timescape.headlessService.labels | object | `{}` | Labels to add to the Hubble Timescape headless service. |
| hubble.timescape.ingester.k8sImporter.clusterRole.annotations | object | `{}` | Annotations to add to the Hubble Timescape K8s importer ClusterRole. |
| hubble.timescape.ingester.k8sImporter.clusterRole.labels | object | `{}` | Labels to add to the Hubble Timescape K8s importer ClusterRole. |
| hubble.timescape.ingester.k8sImporter.clusterRoleBinding.annotations | object | `{}` | Annotations to add to the Hubble Timescape K8s importer ClusterRoleBinding. |
| hubble.timescape.ingester.k8sImporter.clusterRoleBinding.labels | object | `{}` | Labels to add to the Hubble Timescape K8s importer ClusterRoleBinding. |
...
| hubble.timescape.labels | object | `{}` | Labels to add to all Hubble Timescape resources. |
...
| hubble.timescape.persistence.annotations | object | `{}` | Annotations to add to the Hubble Timescape persistent volume claim. |
...
| hubble.timescape.persistence.labels | object | `{}` | Labels to add to the Hubble Timescape persistent volume claim. |
...
| hubble.timescape.serviceAccount.annotations | object | `{}` | Annotations to add to the Hubble Timescape service account. |
| hubble.timescape.serviceAccount.labels | object | `{}` | Labels to add to the Hubble Timescape service account. |
| hubble.timescape.statefulSet.annotations | object | `{}` | Annotations to add to the Hubble Timescape StatefulSet. |
| hubble.timescape.statefulSet.labels | object | `{}` | Labels to add to the Hubble Timescape StatefulSet. |
| hubble.timescape.tls.auto.role.annotations | object | `{}` | Annotations to add to the Hubble Timescape certificate generation Role. |
| hubble.timescape.tls.auto.role.labels | object | `{}` | Labels to add to the Hubble Timescape certificate generation Role. |
| hubble.timescape.tls.auto.roleBinding.annotations | object | `{}` | Annotations to add to the Hubble Timescape certificate generation RoleBinding. |
| hubble.timescape.tls.auto.roleBinding.labels | object | `{}` | Labels to add to the Hubble Timescape certificate generation RoleBinding. |
| hubble.timescape.tls.client.certificate.annotations | object | `{}` | Annotations to add to the generated Hubble Timescape client certificate resource. |
| hubble.timescape.tls.client.certificate.labels | object | `{}` | Labels to add to the generated Hubble Timescape client certificate resource. |
...
| hubble.timescape.tls.server.certificate.annotations | object | `{}` | Annotations to add to the generated Hubble Timescape server certificate resource. |
| hubble.timescape.tls.server.certificate.labels | object | `{}` | Labels to add to the generated Hubble Timescape server certificate resource. |
