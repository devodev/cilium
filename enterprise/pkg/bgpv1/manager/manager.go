...
// configuration info.
type LocalInstanceMap map[string]*instance.EnterpriseBGPInstance

type enterpriseBGPRouterManagerParams struct {
	cell.In
...
type State struct {
	...
	reconcilers []reconcilerv2.EnterpriseStateReconciler
...
//
// Logically, this manager views each BGP instance within an
// IsovalentBGPNodeConfig as a BGP router instantiated on its host.
//
...
type BGPRouterManager struct {
	...
	ConfigReconcilers []reconcilerv2.EnterpriseConfigReconciler
...
func NewBGPRouterManager(params enterpriseBGPRouterManagerParams) agent.EnterpriseBGPRouterManager {
	...
	m := &BGPRouterManager{
		...
		// state
		state: State{
			reconcilers:            reconcilerv2.GetActiveEnterpriseStateReconcilers(params.Logger, params.StateReconcilers),
	...
	params.JobGroup.Add(
		job.OneShot("enterprise-bgp-state-observer", func(ctx context.Context, health cell.Health) (err error) {
			for {
				select {
				case <-ctx.Done():
					return nil
...
func (m *BGPRouterManager) GetPeers(ctx context.Context, req *ossAgent.GetPeersRequest) (*ossAgent.GetPeersResponse, error) {
	...
	var res ossAgent.GetPeersResponse
	for _, i := range m.BGPInstances {
		r, err := i.Router.GetPeerState(ctx, &types.GetPeerStateRequest{})
		if err != nil {
			return nil, err
		}
		res.Instances = append(res.Instances, ossAgent.InstancePeerStates{
			Name:  i.Name,
			Peers: r.Peers,
		})
	}
...
func (m *BGPRouterManager) GetRoutes(ctx context.Context, req *ossAgent.GetRoutesRequest) (*ossAgent.GetRoutesResponse, error) {
	m.RLock()
	defer m.RUnlock()
	...
	var res ossAgent.GetRoutesResponse
	for _, i := range m.BGPInstances {
		...
		case types.TableTypeAdjRIBIn, types.TableTypeAdjRIBOut:
			...
			for _, peer := range peerState.Peers {
				...
				res.Instances = append(res.Instances, ossAgent.InstanceRoutes{
					InstanceName: i.Name,
...
func (m *BGPRouterManager) getRoutesFromInstanceLegacy(ctx context.Context, i *instance.EnterpriseBGPInstance, params restapi.GetBgpRoutesParams) ([]*models.BgpRoute, error) {
...
func (m *BGPRouterManager) GetRoutePolicies(ctx context.Context, params *ossAgent.GetRoutePoliciesRequest) (*ossAgent.GetRoutePoliciesResponse, error) {
	m.RLock()
	defer m.RUnlock()
	...
	var res ossAgent.GetRoutePoliciesResponse
	for _, i := range m.BGPInstances {
		...
		res.Instances = append(res.Instances, ossAgent.InstanceRoutePolicies{
			Name:          i.Name,
...
func (m *BGPRouterManager) ReconcileInstances(ctx context.Context,
	...
	return m.reconcileEnterpriseInstances(ctx, toIsovalentBGPNodeConfig(nodeObj), ciliumNode)
}

func (m *BGPRouterManager) ReconcileEnterpriseInstances(ctx context.Context,
	nodeObj *v1.IsovalentBGPNodeConfig,
	ciliumNode *v2.CiliumNode) error {
	return m.reconcileEnterpriseInstances(ctx, nodeObj, ciliumNode)
}

// reconcileEnterpriseInstances is an API for configuring the BGP Instances from
// the desired IsovalentBGPNodeConfig resource.
//
// reconcileEnterpriseInstances will evaluate BGP instances to be created,
// removed and reconciled.
func (m *BGPRouterManager) reconcileEnterpriseInstances(ctx context.Context,
	nodeObj *v1.IsovalentBGPNodeConfig,
	ciliumNode *v2.CiliumNode) error {
	m.Lock()
	...
	if rd.empty() {
		m.logger.Debug("BGP instance up-to-date with IsovalentBGPNodeConfig", types.BGPNodeConfigLogField, nodeObj.Name)
		return nil
	}
...
func (m *BGPRouterManager) register(ctx context.Context, rd *reconcileDiff) error {
	...
	for _, name := range rd.register {
		var config *v1.IsovalentBGPNodeInstance
		var ok bool
...
func (m *BGPRouterManager) registerBGPInstance(ctx context.Context,
	...
	i, err := instance.NewEnterpriseBGPInstance(ctx, m.routerProvider, l, c.Name, globalConfig)
	if err != nil {
...
func (m *BGPRouterManager) reconcileBGPConfig(ctx context.Context,
	i *instance.EnterpriseBGPInstance,
	newc *v1.IsovalentBGPNodeInstance,
	ciliumNode *v2.CiliumNode) error {
	...
	for _, r := range m.ConfigReconcilers {
		if rErr := r.Reconcile(ctx, reconcilerv2.EnterpriseReconcileParams{
			BGPInstance:   i,
			DesiredConfig: newc,
...
func getLocalASN(config *v1.IsovalentBGPNodeInstance) (int64, error) {
...
func getRouterID(config *v1.IsovalentBGPNodeInstance, ciliumNode *v2.CiliumNode) (string, error) {
	...
	case option.BGPRouterIDAllocationModeIPPool:
		if config.RouterID == nil {
			return "", fmt.Errorf("can't find the router-id in the IsovalentBGPNodeInstance")
		}
...
func getLocalPort(config *v1.IsovalentBGPNodeInstance) (int32, error) {
...
func toIsovalentBGPNodeConfig(in *v2.CiliumBGPNodeConfig) *v1.IsovalentBGPNodeConfig {
	...
	out := &v1.IsovalentBGPNodeConfig{
		TypeMeta:   in.TypeMeta,
		ObjectMeta: in.ObjectMeta,
		Spec: v1.IsovalentBGPNodeSpec{
			BGPInstances: make([]v1.IsovalentBGPNodeInstance, 0, len(in.Spec.BGPInstances)),
		},
	}
	for i := range in.Spec.BGPInstances {
		out.Spec.BGPInstances = append(out.Spec.BGPInstances, toIsovalentBGPNodeInstance(&in.Spec.BGPInstances[i]))
	}
	return out
}

func toIsovalentBGPNodeInstance(in *v2.CiliumBGPNodeInstance) v1.IsovalentBGPNodeInstance {
	out := v1.IsovalentBGPNodeInstance{
		Name:      in.Name,
		LocalASN:  in.LocalASN,
		RouterID:  in.RouterID,
		LocalPort: in.LocalPort,
		Peers:     make([]v1.IsovalentBGPNodePeer, 0, len(in.Peers)),
	}
	for _, peer := range in.Peers {
		out.Peers = append(out.Peers, toIsovalentBGPNodePeer(peer))
	}
	return out
}

func toIsovalentBGPNodePeer(in v2.CiliumBGPNodePeer) v1.IsovalentBGPNodePeer {
	out := v1.IsovalentBGPNodePeer{
		Name:         in.Name,
	...
	if in.PeerConfigRef != nil {
		out.PeerConfigRef = &v1.PeerConfigReference{
			Name: in.PeerConfigRef.Name,
		}
	}
	if in.AutoDiscovery != nil && in.AutoDiscovery.Mode == v2.BGPDefaultGatewayMode {
		out.AutoDiscovery = &v1.BGPAutoDiscovery{
			Mode:           v1.BGPADDefaultGateway,
			DefaultGateway: in.AutoDiscovery.DefaultGateway,
		}
	}
	return out
}
...
func (m *BGPRouterManager) GetRoutesExtended(ctx context.Context, req *agent.GetRoutesExtendedRequest) (*agent.GetRoutesExtendedResponse, error) {
	...
	for _, i := range m.BGPInstances {
		...
		case types.TableTypeAdjRIBIn, types.TableTypeAdjRIBOut:
			...
			for _, peer := range peerState.Peers {
				rs, err := i.Router.GetRoutesExtended(ctx, &entTypes.GetRoutesExtendedRequest{
					GetRoutesRequest: types.GetRoutesRequest{
						TableType: req.TableType,
						Family:    req.Family,
						Neighbor:  peer.Address,
					},
...
// GetRoutePoliciesExtended returns BGP routing policies of the specified BGP instance.
func (m *BGPRouterManager) GetRoutePoliciesExtended(ctx context.Context, instanceName string) (map[string][]*entTypes.ExtendedRoutePolicy, error) {
	m.RLock()
	defer m.RUnlock()
	...
	res := make(map[string][]*entTypes.ExtendedRoutePolicy)

	for _, i := range m.BGPInstances {
		if instanceName != "" && i.Name != instanceName {
			continue
		}
		resp, err := i.Router.GetRoutePoliciesExtended(ctx)
		if err != nil {
			return nil, err
		}
