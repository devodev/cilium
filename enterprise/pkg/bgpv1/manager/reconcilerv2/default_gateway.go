...
func NewDefaultGatewayReconciler(p DefaultGatewayReconcilerIn) DefaultGatewayReconcilerOut {
	if !p.BGPConfig.Enabled {
		return DefaultGatewayReconcilerOut{}
	}

	logger := p.Logger.With(types.ReconcilerLogField, DefaultGatewayReconcilerName)

	// Add job observers for route and device change tracking
	p.JobGroup.Add(
		job.Observer("enterprise-default-gateway-route-change-tracker",
			routeChangeTrackerObserver(p.Signaler, logger),
			statedb.Observable(p.DB, p.RouteTable)),
	)

	p.JobGroup.Add(
		job.Observer("enterprise-default-gateway-device-change-tracker",
			deviceChangeTrackerObserver(p.Signaler, logger),
			statedb.Observable(p.DB, p.DeviceTable)),
	)

	return DefaultGatewayReconcilerOut{
		EnterpriseReconciler: &DefaultGatewayReconciler{
			logger:      logger,
			DB:          p.DB,
...
func (r *DefaultGatewayReconciler) Init(i *instance.EnterpriseBGPInstance) error {
	if i == nil {
...
func (r *DefaultGatewayReconciler) Cleanup(i *instance.EnterpriseBGPInstance) {
}

func (r *DefaultGatewayReconciler) Reconcile(ctx context.Context, p EnterpriseReconcileParams) error {
	...
	for i, peer := range p.DesiredConfig.Peers {
		...
		// Enterprise AutoDiscovery also includes Unnumbered mode. It is handled
		// by LinkLocalReconciler, so this reconciler only processes DefaultGateway.
		if peer.AutoDiscovery.Mode != v1.BGPADDefaultGateway {
			continue
		}
