...
func BGPPeersCmd(bgpMgr agent.EnterpriseBGPRouterManager) script.Cmd {
	return script.Command(
		...
		func(s *script.State, args ...string) (script.WaitFunc, error) {
			return func(*script.State) (stdout, stderr string, err error) {
				...
				res, err := bgpMgr.GetPeersExtended(s.Context(), &agent.GetPeersExtendedRequest{})
				if err != nil {
					return "", "", err
				}
...
func PrintPeerStatesTable(w io.Writer, instances []agent.InstancePeerStatesExtended, noUptime bool) {
	...
	for i, row := range rows {
		// Always print header
		if i != 0 {
			established := row.SessionState == ossTypes.SessionEstablished.String()
...
func PrintPeerStatesDetailed(w io.Writer, instances []agent.InstancePeerStatesExtended, noUptime bool) {
	slices.SortFunc(instances, func(a, b agent.InstancePeerStatesExtended) int {
	...
	for _, instance := range instances {
		...
		for _, peer := range instance.Peers {
			...
			slices.SortFunc(peer.Families, func(a, b ossTypes.PeerFamilyState) int {
