...
func generate() {
	...
	unsupportedFeatures := map[string]features.YAMLFeature{}
	for id, feat := range features.FeaturesYaml.Features {
		if len(feat.Helm) == 0 && len(feat.HelmCheck) == 0 {
			continue
		}
		if feat.Maturity == features.UnsupportedMaturity {
			unsupportedFeatures[id] = feat
		} else if feat.Maturity != stable {
...
func validate() {
	...
	for id, feat := range features.FeaturesYaml.Features {
		go func() {
			...
			// Unsupported features must always fail, even when approved.
			if feat.Maturity == features.UnsupportedMaturity {
				cmd = exec.Command(helmProg,
					append(defArgs,
						"--set",
				...
				if err := cmd.Run(); err == nil {
					results <- result{
						id,
						fmt.Errorf("expected Unsupported feature %s to fail even when approved (%s)", id, buf.String()),
					}
					return
				}
				results <- result{id, nil}
...
[[- $fails := false -]]
{{- range $id, $feat := .UnsupportedFeatures}}
[[- if {{ index $helmConds $id }} -]]
  [[- fail "\nUnsupported feature: {{$id}} cannot be enabled. Please contact Isovalent Support for more information." -]]
[[- end -]]
{{- end -}}
